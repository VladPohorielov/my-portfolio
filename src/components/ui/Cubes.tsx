/**
 * Cubes — інтерактивний 3D-куб фон.
 * Адаптовано з react-bits (reactbits.dev/backgrounds/cubes).
 * Tailwind-класи замінено на inline-стилі та CSS-модуль.
 * Залежності: gsap
 */
...
const Cubes: React.FC<CubesProps> = ({
  ...
  cellGap,
  borderStyle = "1px solid rgba(99,102,241,0.15)",
  faceColor = "#09090b",
  shadow = false,
  autoAnimate = true,
  rippleOnClick = true,
  ...
  return (
    <div className={s.wrapper} style={wrapperStyle}>
      <div ref={sceneRef} className={s.scene} style={sceneStyle}>
        {cells.map((_, r) =>
          cells.map((__, c) => (
            <div key={`${r}-${c}`} className="cube" data-row={r} data-col={c}
              style={{ position: "relative", width: "100%", height: "100%",
                aspectRatio: "1", transformStyle: "preserve-3d" }}
            >
              <span style={{ position: "absolute", pointerEvents: "none",
                inset: "-36px" }} />
              {/* Front */}
              <div className="cube-face" style={{ ...faceStyle, position: "absolute",
